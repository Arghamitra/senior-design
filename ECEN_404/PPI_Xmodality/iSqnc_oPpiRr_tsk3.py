"""
Task 3: Input sequences  => output binary PPI +/- AND binary interprotein residue contacts
Use task 1 Siamese HRNN encoders trained using all +/- sequences (as the pre-training stage)
to warm start parameter training, and loss is now PPI cross entropy (we might drop it or not) +
 interprotein residue contact cross entropy (based on joint attentions) +
 weighted sparsity (L1 norm) for joint attentions.
  Same model as in Task 1. You only have positive PPIs with inter-protein complex structures for the fine-tuning.
...
class net_crossInteraction_old(nn.Module):
...
class net_crossInteraction(nn.Module):
    ...
    def forward(self, prot_data1, prot_data2, label, inter_prot_contacts):
        ...
        """
        inter_prot_prot = self.sigmoid(torch.einsum('bij,bkj->bik', self.joint_attn_prot1(self.relu(prot_embedding1)),
                                                                self.joint_attn_prot2(self.relu(prot_embedding2))))
        """
...
model_old = net_crossInteraction_old(args.l0, args.l1, args.l2, args.l3)
#model_old = nn.DataParallel(model_old)
model_old = model_old.cuda()
model_old.load_state_dict(torch.load('./weights/concatenation_drpot_' + str(dropout) + '_stpsz_' + str(step_size) + '.pth'))
...
if args.train == 1:
    ...
    #for epoch in tqdm(range(args.epoch)):
        ...
        for prot_data1, prot_data2, label, INTER_prot_contact, INTRA_prot_contact1, INTRA_prot_contact2 in train_loader:
            ...
            optimizer.zero_grad()
            loss = model(prot_data1, prot_data2, label, INTER_prot_contact).mean()
        ...
        if AUPRC > AUPRC_val_best:
            ...
            #torch.save(model.state_dict(),
                       './weights/iSqnc_oPpiRr_tsk3_cnctntn_drpot_' + str(dropout) + '_stpsz_' + str(step_size) +'_epoch_' + str(args.epoch)+ '_' + str(args.l1)+ '.pth')
...
model.load_state_dict(
    torch.load('./weights/iSqnc_oPpiRr_tsk3_cnctntn_drpot_' + str(dropout) + '_stpsz_' + str(step_size) +'_epoch_' + str(args.epoch)+ '_' + str(args.l1)+ '.pth'))
model.eval()
