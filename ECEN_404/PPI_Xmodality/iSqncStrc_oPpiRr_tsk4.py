...
parser.add_argument('--data_processed_dir', type=str,
                    default=('/home/argha/WORK/extracted_data/extracted_data/final_data_2D/'))
...
class net_crossInteraction(nn.Module):
    def __init__(self, lambda_l1, lambda_fused, lambda_group, lambda_bind):
        ...
        self.gat_n = net_prot_gat()  ##### GAT
        self.crossInteraction = crossInteraction()  ########
        # self.gcn_comp = net_comp_gcn() ###########

        self.relu_n = nn.ReLU()
        self.sigmoid_n = nn.Sigmoid()
        self.joint_attn_prot1_n, self.joint_attn_prot2_n = nn.Linear(128, 128), nn.Linear(128, 128)
        self.tanh = nn.Tanh()

        self.regressor0_n = nn.Sequential(nn.Conv1d(1, 64, kernel_size=4, stride=2, padding=1),
                                          nn.LeakyReLU(0.1),
                                          nn.MaxPool1d(kernel_size=4, stride=4))
        self.regressor1_n = nn.Sequential(nn.Linear(64 * 16, 600),
                                          nn.LeakyReLU(0.1),
                                          nn.Dropout(dropout),
                                          nn.Linear(600, 300),
                                          nn.LeakyReLU(0.1),
    ...
    def forward(self, prot_data1, prot_data2, INTRA_prot_contact1, INTRA_prot_contact2, inter_prot_contacts):
        ...
        prot_graph_embedding1 = self.gat_n(prot_graph_embedding1, INTRA_prot_contact1)
        ...
        prot_graph_embedding2 = self.gat_n(prot_graph_embedding2, INTRA_prot_contact2)
        ...
        inter_prot_prot = self.sigmoid_n(torch.einsum('bij,bkj->bik', self.joint_attn_prot1_n(self.relu_n(prot_embedding1)),
                                                    self.joint_attn_prot2_n(self.relu_n(prot_embedding2))))
        ...
        affn_prot_prot = self.regressor0_n(affn_prot_prot)
        ...
        loss0 = self.loss_reg(inter_prot_prot_masked)
        loss1 = self.loss_affn(inter_prot_prot_masked, inter_prot_contacts)
        loss = loss0 + loss1
        return loss

    def forward_inter_affn(self, prot_data1, prot_data2, INTRA_prot_contact1, INTRA_prot_contact2):
        # finds out the length of each protein sequenc in a batch without padding
        len_prot_data1 = self.find_pro_len(prot_data1)
        len_prot_data2 = self.find_pro_len(prot_data2)
        ...
        inter_prot_prot = self.sigmoid_n(
            torch.einsum('bij,bkj->bik', self.joint_attn_prot1_n(self.relu_n(prot_embedding1)),
                         self.joint_attn_prot2_n(self.relu_n(prot_embedding2))))
        inter_prot_prot_sum = torch.einsum('bij->b', inter_prot_prot)
...
class net_comp_gcn(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear = nn.ModuleList([nn.Linear(43, 128), nn.Linear(128, 128), nn.Linear(128, 128)])
        self.relu = nn.ReLU()
        self.linear_final = nn.Linear(128, 128)
...
class dataset(torch.utils.data.Dataset):
    def __init__(self, name_split='train'):
        ...
        self.prot_data1, self.prot_data2, self.label, self.INTER_prot_contact, self.INTRA_prot_contact1, self.INTRA_prot_contact2 = torch.tensor(
            ...
            self.INTER_prot_contact).float(), torch.tensor(self.INTRA_prot_contact1).float(), torch.tensor(
            self.INTRA_prot_contact2).float()
...
def calculate_AUPRC(model, loader, batch_size, logging=False, logpath=''):
    ...
    for prot_data1, prot_data2, label, INTER_prot_contact, INTRA_prot_contact1, INTRA_prot_contact2 in loader:
        ...
        with torch.no_grad():
            #inter, _, Len_prot_data1, Len_prot_data2 = model.module.forward_inter_affn(
            inter, _, Len_prot_data1, Len_prot_data2=model.forward_inter_affn(
                prot_data1, prot_data2, INTRA_prot_contact1, INTRA_prot_contact2)
    ...
    for ix in range(int(b)):
        #labels has all the inter contact maps
        trth = labels[ix]

        #we are only taking the part associated with protein length 1 and protein length 2
...
checkpoint_pth = 'iSqncStrc_oPpiRr_tsk4_NN_cnctntn_drpot_' + str(dropout) + '_stpsz_' + str(step_size) + '_chechpoint.pth'
...
if args.train == 1:
    ...
    for epoch in (range(start_epoch, args.epoch)):
        ...
        for prot_data1, prot_data2, label, INTER_prot_contact, INTRA_prot_contact1, INTRA_prot_contact2 in train_loader:
            ...
            optimizer.zero_grad()
            loss = model(prot_data1, prot_data2, INTRA_prot_contact1, INTRA_prot_contact2, INTER_prot_contact).mean()
        ...
        if AUPRC > AUPRC_val_best:
            ...
            torch.save(model.state_dict(),
                       './weights/iSqncStrc_oPpiRr_tsk4_NN_' + str(dropout) + '_stpsz_' + str(
                           step_size) + '_epoch_' + str(args.epoch) + '_' + str(args.l1) + '.pth')
...
cal_affinity_torch(model, eval_loader, (args.batch_size), eval_phs="TRAIN", task=4)

print('test')
eval_set = dataset('test')
eval_loader = torch.utils.data.DataLoader(dataset=eval_set, batch_size=args.batch_size, shuffle=False)
cal_affinity_torch(model, eval_loader, (args.batch_size), eval_phs="TEST", task=4)

print('val')
eval_set = dataset('val')
eval_loader = torch.utils.data.DataLoader(dataset=eval_set, batch_size=args.batch_size, shuffle=False)
cal_affinity_torch(model, eval_loader, (args.batch_size), eval_phs="VAL", task=4)
